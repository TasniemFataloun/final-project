import { importCircle, importRectangle, importSquare } from "../config/importElementsProperties.config";
...
export const UseGenerateKeyframes = (config: AnimationType) => {
  ...
  const defaultValues = {
    circle: {
      size: { width: importCircle.size.width, height: importCircle.size.height },
      transform: { scale: importCircle.transform.scale, rotate: importCircle.transform.rotate, translateX: importCircle.transform.translateX, translateY: importCircle.transform.translateY },
      opacity: { opacity: importCircle.opacity.opacity, borderRadius: importCircle.opacity.borderRadius },
      backgroundColor: importCircle.backgroundColor.backgroundColor,
    },
    rectangle: {
      size: { width: importRectangle.size.width, height: importRectangle.size.height },
      transform: { scale: importRectangle.transform.scale, rotate: importRectangle.transform.rotate, translateX: importRectangle.transform.translateX, translateY: importRectangle.transform.translateY },
      opacity: { opacity: importRectangle.opacity.opacity, borderRadius: importRectangle.opacity.borderRadius },
      backgroundColor: importRectangle.backgroundColor.backgroundColor,
    },
    square: {
      size: { width: importSquare.size.width, height: importSquare.size.height },
      transform: { scale: importSquare.transform.scale, rotate: importSquare.transform.rotate, translateX: importSquare.transform.translateX, translateY: importSquare.transform.translateY },
      opacity: { opacity: importSquare.opacity.opacity, borderRadius: importSquare.opacity.borderRadius },
      backgroundColor: importSquare.backgroundColor.backgroundColor,
    },
  };

  // Select the default configuration based on the element type
  const defaultConfig = defaultValues[config.type as keyof typeof defaultValues];
  ...
  const toBorderRadius = opacity.borderRadius || defaultConfig.opacity.borderRadius;
